		...
		for( i = 0 ; i < per_node ; i++ ){
			if (strcmp(flist + (NAME_MAX * i) , "") != 0 ) {
				...
				if (strcmp(optarg.program, "ltr") == 0) {
					// NOT IMPLEMENTED
					sprintf(tmp, "%s/mgescan/ltr/find_ltr_pair.pl -path_genome=%s -path_ltr=%s -chr_name=%s -run_hmm=%d", getenv("MGESCAN_HOME"), optarg.genome, optarg.data, flist + (NAME_MAX * i), 1);
				} else { 
					sprintf(tmp, "%s/mgescan/nonltr/run_hmm.pl --dna=%s/%s --out=%s --hmmerv=%d", getenv("MGESCAN_HOME"), optarg.genome, flist + (NAME_MAX * i), optarg.data, optarg.hmmerv);
				}
				printf("%s\n",tmp);
				res = system(tmp);
				//printf ("%d", res);
