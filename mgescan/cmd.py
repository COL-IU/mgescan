...
class MGEScan(object):
    ...
    def get_env(self):
        ...
        if not os.environ.get("MGESCAN_HOME"):
            print ("MGESCAN_HOME environment variable is not defined or .mgescanrc is not sourced.")
            print (os.environ.get("HOME")+"/mgescan4 (default path) is used"+ \
                    " to run MGEScan")
            mgescan_home = os.environ.get("HOME")+"/mgescan4"
        else:
            mgescan_home = os.environ.get("MGESCAN_HOME")
            print (mgescan_home+" (MGESCAN_HOME) is used"+ \
                    " to run MGEScan")

        if not os.path.exists(mgescan_home):
            print (mgescan_home + " does not exist." + \
                    " MGEScan stopped")
            sys.exit(-1)

        self.base_path = mgescan_home + "/mgescan"
...
def main():
    arguments = docopt(__doc__, version='MGEScan 4.0.0')
